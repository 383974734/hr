...
Page({
  ...
  loadData: function (e) {
    ...
    util.requestPost(api.getMyJobRecruit, data, token).then(
      res => {
        wx.hideLoading();
   ...
   * cell点击事件
   */
  cellpush: function (res) {
    // wx.navigateTo({
    //   url: '../recruitcon/recruitcon',
    // })
  },
  /**
  * 搜索点击完成是触发事件
  ...
  */
  ...
  pushrecharge: function (e) {
    ...
    var data = {
      ...
      jobId: this.data.deleResumeid,
    }
    var token = wx.getStorageSync('token')
    util.requestPost(api.getDeleteMyJobRecruit, data, token).then(
      res => {
  ...
  addbtn: function (res) {
    ...
    if (this.data.businessAuth == '2') {
      wx.navigateTo({
        url: '../recruitadd/recruitadd?jobCompany=' + this.data.jobCompany,
      })
    }else {
      wx.navigateTo({
        url: '../authentication/authentication?businessAuth=' + this.data.businessAuth,
      })
    }
  },
})